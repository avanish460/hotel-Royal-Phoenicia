...
function Hero() {
  ...
  return (
    <div className="relative h-screen flex items-center justify-center text-center text-white overflow-hidden">
      {/* Background Image */}
      <div
        className={`absolute inset-0 bg-cover bg-center transition-all duration-1000 w-130 h-100 sm:w-200 sm:h-100 md:w-full md:h-screen lg:w-full lg:h-screen ${
          isTransitioning
            ? `transform scale-100 translate-x-${direction === 'left' ? '-full' : 'full'} opacity-90`
            : 'transform scale-110 translate-x-0 opacity-100'
        }`}
      ...
      {/* Content Overlay */}
      <div className="relative z-10 px-4 md:px-8 lg:px-16 mb-30 sm:mb-20 md:mb-4 lg:mb-4 text-center">
        <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-8xl font-serif leading-tight">
          Hotel <br/> Royal Phoenicia
        </h1>
        <p className="text-sm md:text-base lg:text-lg md:mb-8 text-gray-300">
          YOUR FAVOURITE PLACE. OUR FAMILY STORY.
        </p>
        {/* You might want to add a button or more content here */}
      </div>

      <div className="absolute bottom-37 md:bottom-8 lg:bottom-8 left-11 z-30 text-gray-300 text-xs font-thin">
        <span>IMPORTANT <br/> INFO</span>
      </div>
